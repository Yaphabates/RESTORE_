...
def load_clip_to_cpu(cfg):
    ...
    design_details = {"trainer": 'DPT',
                "vision_depth": 0,
                "language_depth": 0, "vision_ctx": 0,
                "language_ctx": 0}

    model = clip.build_model(state_dict or model.state_dict(), design_details=design_details)
...
class VPTDeepPromptLearner(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        super().__init__()
        # hyper param
        self.n_ctx = cfg.TRAINER.VPT.N_CTX_VISION
        self.dtype = clip_model.dtype
...
class DPT(TrainerX):
    ...
    def build_model(self):
        ...
        self.class_prompt_num = cfg.TRAINER.SELECTED_COVPT.CPN if cfg.TRAINER.SELECTED_COVPT.CPN < len(classnames) else len(classnames)
        self.pretrain_c = cfg.TRAINER.DPT.PRETRAIN_C
        self.alpha = cfg.TRAINER.DPT.ALPHA
