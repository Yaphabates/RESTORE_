...
class ResidualAttentionBlock_MaPLe(nn.Module):
    ...
    def attention(self, x: torch.Tensor):
        ...
        # flops, params = profile(self.attn, inputs=(x,x,x, ))
        # flops, params = profile(self.mlp, inputs=x)
        # import pdb; pdb.set_trace()
            ...
            if len(compound_prompts_deeper) > 0:
                ...
                if not self.text_layer:
                    ...
                    if not (counter > len(compound_prompts_deeper) - 1):
                        ...
                        # Create/configure learnable tokens of this layer
                        textual_context = compound_prompts_deeper[counter]   ### [n_ctx, 512]
...
class Transformer(nn.Module):
    def __init__(self, width: int, layers: int, heads: int, attn_mask: torch.Tensor = None, prompts_needed=0,
        ...
        if current_trainer == 'IVLP' or current_trainer == 'VPT':
            ...
            # from thop import profile
            # x = torch.randn(size=(77, 51, 768))
            # flops, params = profile(self.resblocks, inputs=(x, ))
            # import pdb; pdb.set_trace()
        elif current_trainer == 'MaPLe' or current_trainer == 'MaPLe_FSA':
            # import pdb; pdb.set_trace()
            ...
            # assert current_trainer == 'CoOp' or current_trainer == 'CoCoOp' or current_trainer == 'DPT' or current_trainer == "TaskRes"
...
class VisionTransformer_MaPLe_FSA(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int, output_dim: int,
                 design_details):
        super().__init__()
        self.input_resolution = input_resolution
...
class CLIP(nn.Module):
    def __init__(self,
        ...
        if isinstance(vision_layers, (tuple, list)):
            ...
            elif trainer == 'MaPLe_FSA':
                self.visual = VisionTransformer_MaPLe_FSA(
                    input_resolution=image_resolution,
                    patch_size=vision_patch_size,
                    width=vision_width,
                    layers=vision_layers,
